...
export default observer(
  class ImageView extends Component {
    ...
    keyDownShortCut = (event) => {
      ...
      // 37, 38, 39, 40 keycodes for left, top, right, bottom
      // not doing since these are few browser shortcuts to navigate to forward and backwork sites.

      if (event.ctrlKey && event.altKey && keyCode === 39) {
        this.props.item.createShortcutRegion("right");
        // console.log("DrawingTool ==> ", this.props.item);
      }
      if (event.ctrlKey && event.altKey && keyCode === 40) {
        this.props.item.createShortcutRegion("bottom");
        // console.log("DrawingTool ==> ", this.props.item);
      }
      if (event.ctrlKey && event.altKey && keyCode === 37) {
        this.props.item.createShortcutRegion("left");
        // console.log("DrawingTool ==> ", this.props.item);
      }
      if (event.ctrlKey && event.altKey && keyCode === 38) {
        this.props.item.createShortcutRegion("top");
        // console.log("DrawingTool ==> ", this.props.item);
      }
    };
    ...
    componentWillUnmount() {
      ...
      document.removeEventListener("keydown", this.keyDownShortCut);
