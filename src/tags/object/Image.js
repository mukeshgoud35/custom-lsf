...
const Model = types
  ...
  .actions((self) => ({
    ...
    createShortcutRegion(copyToSide) {
      const { annotationStore } = self;
      const {
      ...
      if (selectedRegions.length > 0) {
        ...
        const resultValue = result.from_name.getResultValue();
        const value = {
          coordstype,
          height,
          rotation,
        ...
        const currentArea = self.createDrawingRegion(value, resultValue, control, false);
        const newArea = self.annotation.createResult(value, currentArea.results[0].value.toJSON(), control, self);
